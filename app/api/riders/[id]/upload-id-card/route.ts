...
export async function POST(
  ...
  try {
    ...
    const path = `${profile.stable_id}/riders/${riderId}/id.${ext}`;
    ...
    // Store storage path only — bucket is private; clients use authenticated view route.
    await supabase
      .from("riders")
    ...
    return NextResponse.json({
      path,
      url: signed ?? `/api/riders/${riderId}/id-card`,
    });
  } catch (err) {
