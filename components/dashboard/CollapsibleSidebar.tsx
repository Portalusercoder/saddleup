...
export default function CollapsibleSidebar() {
  ...
  return (
    <aside className="h-full w-56 bg-base border-r border-black/10 flex flex-col overflow-hidden dark:border-white/10">
        ...
        <div className="shrink-0 h-20 px-4 border-b border-black/10 dark:border-white/10 flex items-center gap-3">
          {stable.logoUrl ? (
            <img
              src={stable.logoUrl}
              alt=""
              className="w-10 h-10 rounded object-cover border border-black/10 shrink-0 dark:border-white/15"
            />
          ) : (
            <div className="w-10 h-10 rounded bg-black/5 border border-black/10 flex items-center justify-center shrink-0 dark:bg-white/5 dark:border-white/15">
      ...
      <nav className="flex-1 px-2 py-4 overflow-y-auto" aria-label={t("navSection.sidebarAria")}>
        {sections.map((section, sectionIndex) => (
          <div
            key={section.sectionKey}
            className={sectionIndex > 0 ? "mt-5 pt-4 border-t border-black/10 dark:border-white/10" : ""}
          >
            <p className="px-3 mb-2 text-[0.65rem] font-medium uppercase tracking-[0.22em] text-black/45 dark:text-white/45">
              ...
              {section.items.map((item) => {
                ...
                return (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium transition-colors border-s-[3px] ${
                        isActive
                          ? "border-accent bg-black/[0.07] text-black dark:bg-white/10 dark:text-white"
                          : "border-transparent text-black/65 hover:bg-black/[0.04] hover:text-black dark:text-white/65 dark:hover:bg-white/5 dark:hover:text-white"
                      }`}
                    >
